...
const ServiceList = ({ t }) => {
    ...
    const getListData = async (values = searchValues) => {
        ...
        try {
            const { data } = await RestApi.get('api/v1/admin/configurations/driving-related-service-fees/list', { params })
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/driving-related-service-fees/delete/${data.id}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm) => {
        ...
        try {
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/driving-related-service-fees/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/driving-related-service-fees/create', values)
            }
            ...
            <div className=" text-slate-700 card bg-white shadow-md rounded-xl">
                <div className='row m-1'>
                    <div className="col-md-8 col-sm-12">
                        <h3 className="text-lg font-semibold text-slate-800">{t('drivingRelatedServiceFees')}</h3>
