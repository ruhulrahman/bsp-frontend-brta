...
const FuelTypeList = ({ t }) => {
    ...
    const getListData = async (values = searchValues) => {
        ...
        try {
            const { data } = await RestApi.get('api/v1/admin/configurations/fuel-type/list', { params })
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/fuel-type/delete/${data.id}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm) => {
        ...
        try {
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/fuel-type/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/fuel-type/create', values)
            }
...
export default withNamespaces()(FuelTypeList)
