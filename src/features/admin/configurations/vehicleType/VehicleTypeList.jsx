...
import { setListData, setLoading, toggleShowFilter, getCommonDropdowns, updateAndFetchDropdowns } from '@/store/commonSlice';
...
const VehicleTypeList = ({ t }) => {
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/vehicle-type/delete/${data.id}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm, setErrors) => {
        ...
        try {
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/vehicle-type/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/vehicle-type/create', values)
            }
            ...
            if (result.data.success) {
                ...
                handleCloseModal();
                getListData()
                // Fetch updated dropdown values and update the Redux store
                dispatch(updateAndFetchDropdowns({}))
            }
            ...
            <div className=" text-slate-700 card bg-white shadow-md rounded-xl">
                <div className='row m-1'>
                    <div className="col-md-8 col-sm-12">
                        <h3 className="text-lg font-semibold text-green-600">{t('vehicleTypeList')}</h3>
...
export default withNamespaces()(VehicleTypeList)
