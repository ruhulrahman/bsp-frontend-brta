...
const OrganizationList = () => {
    ...
    const getListData = async (values = searchValues) => {
        ...
        try {
            const { data } = await RestApi.get('api/v1/admin/configurations/organization/list', { params })
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/organization/delete/${data.orgId}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm, setErrors) => {
        ...
        try {
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/organization/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/organization/create', values)
            }
                                        ...
                                        <div className="col-md-3 col-sm-12">
                                            <Form.Group className="mb-3" controlId="officeTypeId">
                                                <Field
                                                    ...
                                                    placeholder={t('selectOfficeType')}
                                        ...
                                        {/* <div className="col-md-3 col-sm-12">
                                            <Form.Group className="mb-3" controlId="divisionId">
                                                <Field
                                                    ...
                                                    onChange={(option) => {
                                                        setFieldValue('divisionId', option ? option.value : '')
                                                    }}
                                                />
                                            </Form.Group>
                                        </div>
            ...
            <div className=" text-slate-700 card bg-white shadow-md rounded-xl">
                <div className='row m-1'>
                    <div className="col-md-8 col-sm-12">
                        <h3 className="text-lg font-semibold text-green-600">{t('organizationList')}</h3>
                        ...
                        <tbody>
                            ...
                            {listData && listData.map((item, index) => (
                                <tr key={item.id} className='text-slate-500 text-sm'>
                                    ...
                                    <td>{item.nameEn}</td>
                                    <td>{item.nameBn}</td>
                                    <td>{currentLanguage === 'en' ? item.officeTypeNameEn : item.officeTypeNameBn}</td>
                                    <td>{currentLanguage === 'en' ? item.fullAddressEn : item.fullAddressBn}</td>
                                    <td>
