...
const VehicleBrandList = ({ t }) => {
    ...
    const getListData = async (values = searchValues) => {
        ...
        try {
            const { data } = await RestApi.get('api/v1/admin/configurations/vehicle-brand/list', { params })
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/vehicle-brand/delete/${data.id}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm) => {
        ...
        try {
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/vehicle-brand/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/vehicle-brand/create', values)
            }
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="col-md-3 col-sm-12">
                                            <Form.Group className="mb-3" controlId="makerId">
                                                ...
                                                >
                                                    <option value="">{t('selectVehicleMaker')}</option>
                                                    {vehicleMakerList && vehicleMakerList.map((option) => (
            ...
            <div className=" text-slate-700 card bg-white shadow-md rounded-xl">
                <div className='row m-1'>
                    <div className="col-md-8 col-sm-12">
                        <h3 className="text-lg font-semibold text-slate-800">{t('vehicleBrandList')}</h3>
                    ...
                    <div className="col-md-4 col-sm-12 text-right">
                        ...
                        <AddNew
                            ...
                            onSave={handleSave}
                            editData={editData}
                            vehicleMakerList={vehicleMakerList}
                        />
                    </div>
                </div>
                    ...
                    <table className="mt-2 text-left table table-responsive min-w-max">
                        <thead>
                            <tr>
                                ...
                                <th>{t('name') + ` (${t('en')})`}</th>
                                <th>{t('name') + ` (${t('bn')})`}</th>
                                <th className='text-center'>{t('vehicleMaker')}</th>
                                <th>{t('status')}</th>
...
export default withNamespaces()(VehicleBrandList)
