...
const VehicleMakerList = () => {
    ...
    const getListData = async (values = searchValues) => {
        ...
        try {
            const { data } = await RestApi.get('api/v1/admin/configurations/vehicle-maker/list', { params })
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/vehicle-maker/delete/${data.id}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm, setErrors) => {
        ...
        try {
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/vehicle-maker/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/vehicle-maker/create', values)
            }
                        ...
                        <tbody>
                            ...
                            {listData && listData.map((item, index) => (
                                <tr key={item.id} className='text-slate-500 text-sm'>
                                    ...
                                    <td>{item.nameBn}</td>
                                    <td>
                                        {currentLanguage === 'en' ? item?.country?.nameEn : item?.country?.nameBn}
                                    </td>
...
export default (VehicleMakerList)
