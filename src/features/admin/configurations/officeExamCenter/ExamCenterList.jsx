...
const ExamCenterList = ({ t }) => {
    ...
    const getListData = async (values = searchValues) => {
        ...
        try {
            const { data } = await RestApi.get('api/v1/admin/configurations/office-exam-center/list', { params })
    ...
    const deleteData = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(setLoading(true));
                try {
                    await RestApi.delete(`api/v1/admin/configurations/office-exam-center/delete/${data.orgId}`)
    ...
    const handleSave = async (values, setSubmitting, resetForm, setErrors) => {
        ...
        try {
            ...
            let result = ''
            if (values.id) {
                result = await RestApi.put(`api/v1/admin/configurations/office-exam-center/update/${values.id}`, values)
            } else {
                result = await RestApi.post('api/v1/admin/configurations/office-exam-center/create', values)
            }
    ...
    //     try {
    //         const { data } = await RestApi.get('api/v1/admin/configurations/office-exam-center/all-list')
    //         if (data.success) {
            ...
            <div className=" text-slate-700 card bg-white shadow-md rounded-xl">
                <div className='row m-1'>
                    <div className="col-md-8 col-sm-12">
                        <h3 className="text-lg font-semibold text-green-600">{t('examCenterList')}</h3>
                    ...
                    <table className="mt-2 text-left table table-responsive">
                        <thead>
                            <tr>
                                ...
                                <th>{t('office')}</th>
                                <th>{t('thana')}</th>
                                <th>{t('address') + ' (' + t('en') + ')'}</th>
                                <th>{t('address') + ' (' + t('bn') + ')'}</th>
                                <th>{t('status')}</th>
...
export default withNamespaces()(ExamCenterList)
